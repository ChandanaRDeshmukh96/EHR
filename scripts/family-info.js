...
function createPatientData(element, i) {
    ...
    var dataObj = {};
    dataObj.id = i;
    var name = element.relationName.split(" ");
    dataObj.patientChartNum = findChartNo(name[0], name[1]);
    dataObj.relationship = element.relationship;
    dataObj.emergencyContact = element.emergencyContact;
    ...
    csv()
        ...
        .then((jsonObj) => {
            ...
            fs.writeFile('../JSON/' + fileName + '.json', JSON.stringify(formattedData), 'utf8', function (err) {
                if (err) throw err;
                console.log('Family info data created!');
            });
        });
};
