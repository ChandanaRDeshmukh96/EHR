const csvFilePath = '../CSV/patient-report.csv';
...
function createPatientData(element, i) {
    ...
    var dataObj = {};
    dataObj.id = i;
    dataObj.reportName=element.reportName;
    dataObj.description=element.description;
    dataObj.attachment={};
    dataObj.attachment.name="";
    dataObj.attachment.url=element.attachment;
    dataObj.status=element.status;
    dataObj.lastUpdate=element.lastUpdate?element.lastUpdate:"09/01/2018 10:00:00";
...
csv()
    ...
    .then((jsonObj) => {
        ...
        fs.writeFile('../JSON/patient-report.json', JSON.stringify(formattedData), 'utf8', function (err) {
            if (err) throw err;
            console.log('Saved!');
        });
    });